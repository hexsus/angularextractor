...
export function activate(context: vscode.ExtensionContext) {
	console.log('Congratulations, your extension "angularextractor" is now active!');
	const disposable = vscode.commands.registerCommand('angularextractor.helloWorld', () => {
		vscode.window.showInformationMessage('Hello World from AngularExtractor!');
	});

	let extractComponent = vscode.commands.registerCommand('angularextractor.extractComponent', async () => {
...
`;
		...
		const newFilePath = path.join(currentDir, `${componentName}.component.ts`);
